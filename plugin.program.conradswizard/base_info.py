...
PACKAGES            = glo_var.PACKAGES
skin                = xbmc.getSkinDir()
THUMBS              = glo_var.THUMBS
    ...
    if os.path.exists(temp_cache_path)==True:    
        for root, dirs, files in os.walk(temp_cache_path):
            ...
            if file_count > 0:
                ...
                pass
    DIALOG = xbmcgui.Dialog()
    DIALOG.ok(cr+gn+cr2+cr1+" Wizard"+cr2, cr1+" All Cache Files Removed"+cr2, cr1+"Brought To You By Kodiy Help: and "+cr2+cr+ gn+ cr2)
            ...
            if file_count > 0:
                ...
                if DIALOG.yesno(cr1+"Delete Package Cache Files"+cr2, cr3+str(file_count)+cr2 + cr1+" files found", "Do you want to delete them?"+cr2,yeslabel=cr+'YES'+cr2,nolabel=cr1+'NO'+cr2):                      
                    ...
                    for d in dirs:
                        shutil.rmtree(os.path.join(root, d))
                    DIALOG = xbmcgui.Dialog()
                    DIALOG.ok(cr+gn+cr2+cr1+" Wizard"+cr2, cr1+"Packages Successfuly Removed"+cr2, cr1+"Brought To You By KoDiY Help and "+cr2+ cr+gn+cr2)
    except: 
        DIALOG = xbmcgui.Dialog()
        DIALOG.ok(cr+gn+" Wizard"+cr2, cr1+"Sorry we were not able to remove Package Files"+cr2, cr1+"Brought To You By KoDiY Help: and "+cr2+cr1 +gn+cr2)
...
def viewLogFile():
    ...
    if os.path.exists(log) or not log == False:
        ...
        TextBoxes("%s - %s" % (cr+ADDONTITLE+cr2, logtype), msg)
    else: 
...
def Wizard(name,url,description):
    ...
    if fresh:
        ...
        dp.create(cr+gn+cr2+"[COLOR dodgerblue][B] and KoDiY HELP WIZARD[/B][/COLOR]","[COLOR snow][B]The Chosen Build is Whizzing down your Wires[/B][/COLOR] ",'', '[COLOR snow][B]Put the kettle on[/B][/COLOR]')
    ...
    DIALOG = xbmcgui.Dialog()
    DIALOG.ok("[COLOR dodgerblue][B]DOWNLOAD COMPLETE![/B][/COLOR]", '[COLOR snow][B]To ensure all changes are saved you must now close Kodi[/B][/COLOR]', '[COLOR snow][B]Android users can click okay and it WILL force close itself.[/B][/COLOR]')
    killxbmc()

def Wizard_Adult(name,url,description):
    fresh = xbmcgui.Dialog().yesno(cr+gn+" Wizard"+cr2, cr+'                    Would you like to run a Fresh Start?'+cr2, cr1+'Before installing the latest Adult Build?'+cr2, '', yeslabel=cr+'YES'+cr2,nolabel=cr1+'NO'+cr2)
    if fresh:
        BACKUP = Fresh_Start()
    else:    
        path = xbmc.translatePath(os.path.join('special://home/addons','packages'))
